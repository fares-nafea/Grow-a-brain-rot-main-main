...
function Service.createServerModel(player: Player, key: string, data: any)
	...
	local trueName = key:split(":")[1] -- Carrot:s1238s
	...
	if correspondingFolder and saveData and seedData then
		...
		-- Scaling Based on PlantScaling Save [PlantSize]
		...
		for index: number, fruitData: any in data.Fruits do
			...
			-- Updating Folder --> Actual plotData (plotData is the only thing that saves!!)
		...
		deserializedCFrame = CFrame.new(deserializedCFrame.Position)
		
		-- Converting to Global/World CFrame
		deserializedCFrame = Service.getPlot(player).ReferencePoint.CFrame:ToWorldSpace(deserializedCFrame)
	...
	if playerData then
		if action == "seedPlanted" then
			...
			if itemKey then
				...
				if newItemData then
					-- Update the Plot With the New Item
...
function Service.init()
	-- Growing Seeds/Making Fruits
	-- Includes Offline Growing As System is Reliant On [DatePlanted]
